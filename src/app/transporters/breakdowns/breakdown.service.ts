...
export class BreakdownService {
  ...
  search(page: number, perPage: number, search: string): Observable<any> {
    ...
    return this.http.get<any>(
      `${this.API}?type=transporter&page=${page}&perPage=${perPage}&search=${search}&transporter_id=${transporter}`);
  }

  update(id, message): Observable<any> {
    return this.http.put(`${environment.api}changes/${id}`, { message });
  }
}
